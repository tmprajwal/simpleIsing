...
program ising2d
  ...
  integer ::  i,j,k,cout,maxn,inte,mag
 ...
 do maink = 1,101
  ...
  write(*,*) energy, energy2,cv,abs(mag)
  call printmat(s,n,n)
 enddo
...
!--------------------------------------------------------------------------
  ...
  subroutine inien()
    ...
    integer :: dt,db,dl,dr,dtr,dbl,dbr,dtl,ini,inj
    ...
    do ini = 1,n
        do inj = 1,n
            ...
            !write(*,*) dt, dr, db, dl
            ienergy = ienergy + (-s(ini,inj)*(dt+dr+db+dl+dbl+dtr+dbr+dtl))
            !write(*,*) ienergy
       enddo
    enddo
    energy = ienergy/2
...
!--------------------------------------------------------------------------
  ...
  subroutine deltau(di,dj)
    ...
    integer :: dt,db,dl,dr,dtr,dbl,dtl,dbr
    ...
    ediff = (2.0*s(di,dj)*(dt+dr+db+dl+dtr+dbl+dbr+dtl))
